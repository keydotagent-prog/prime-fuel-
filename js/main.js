...
const introHoldMs = introReduceMotion ? 0 : 1100;
setTimeout(() => {
...
const reduceMotion = introReduceMotion;
...
const key = new THREE.SpotLight(0xffffff, 6, 20, Math.PI / 6, 0.4, 1.2);
...
/* ---------- Geometry detail scales with device ---------- */

const seg = isMobile ? 24 : 48;
const sphereSeg = isMobile ? 16 : 32;
const torusSeg = isMobile ? 48 : 100;

/* ---------- Pedestal ---------- */

const pedestalMat = new THREE.MeshStandardMaterial({ color: 0x2a2a2a, roughness: 0.4, metalness: 0.6 });
const pedestal = new THREE.Mesh(new THREE.CylinderGeometry(1.3, 1.5, 0.3, seg), pedestalMat);
...
const lid = new THREE.Mesh(new THREE.CylinderGeometry(0.585, 0.585, 0.22, seg), lidMat);
...
const scoop = new THREE.Mesh(new THREE.SphereGeometry(0.34, sphereSeg, sphereSeg), scoopMat);
scoop.scale.set(1, 0.55, 1);
const scoopHandleMat = new THREE.MeshStandardMaterial({ color: 0x8a8d8f, roughness: 0.4, metalness: 0.7 });
const scoopHandle = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.5, 12), scoopHandleMat);
...
const crystalMat = new THREE.MeshPhysicalMaterial({
  color: 0xffffff, roughness: 0.05, metalness: 0, transmission: isMobile ? 0 : 0.9,
  thickness: 0.4, ior: 1.3, clearcoat: isMobile ? 0 : 1
});
const crystalPiece = new THREE.Mesh(new THREE.IcosahedronGeometry(0.26, 0), crystalMat);

const capsuleGroup = new THREE.Group();
const capDarkMat = new THREE.MeshStandardMaterial({ color: 0x2a2a2a, roughness: 0.3 });
const capWhiteMat = new THREE.MeshStandardMaterial({ color: 0xf2f0eb, roughness: 0.3 });
const capHalf1 = new THREE.Mesh(new THREE.CapsuleGeometry(0.16, 0.42, 4, isMobile ? 8 : 16), capDarkMat);
...
const particleCount = (reduceMotion || isMobile) ? 0 : 160;
let particles;
if (particleCount > 0) {
...
function animate() {
  ...
  const sceneVisible = fadeOut < 0.999;
  canvasEl.style.opacity = String(1 - fadeOut);

  if (!sceneVisible) {
    if (wasSceneVisible) canvasEl.style.visibility = 'hidden';
    wasSceneVisible = false;
    return;
  }
  if (!wasSceneVisible) canvasEl.style.visibility = 'visible';
